...
void VisibilityManager::createBuffers(const std::vector<uint32_t> &indices) {    // TODO: Rename method (createRtBuffers) (or createVisibilityBuffers)
    VulkanUtil::createBuffer(
        physicalDevice,
    ...
    VulkanUtil::createBuffer(
        physicalDevice,
        logicalDevice, sizeof(Sample) * raysPerIteration,       // TODO
        VK_BUFFER_USAGE_TRANSFER_SRC_BIT | VK_BUFFER_USAGE_RAY_TRACING_BIT_NV | VK_BUFFER_USAGE_STORAGE_BUFFER_BIT,
    ...
    VulkanUtil::createBuffer(
        physicalDevice,
        logicalDevice, sizeof(Sample) * MAX_ABS_RAYS,
        VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_RAY_TRACING_BIT_NV | VK_BUFFER_USAGE_STORAGE_BUFFER_BIT,
...
void VisibilityManager::createDescriptorSets(
    VkBuffer indexBuffer, const std::vector<VkBuffer> &uniformBuffers
) {
    /*
...
void VisibilityManager::initRayTracing(
...

    {
        ...
        std::array<VkDescriptorSet, 2> dd = {
            descriptorSets, descriptorSetsABS
        };
        if (vkAllocateDescriptorSets(
        ...
        descriptorSets = dd[0];
        descriptorSetsABS = dd[1];
    }

    createDescriptorSets(indexBuffer, uniformBuffers);
...
void VisibilityManager::createABSDescriptorSets(VkBuffer vertexBuffer) {
    ...
    */
    ...
    descriptorWrites[0].sType = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
    descriptorWrites[0].dstSet = descriptorSetsABS;
    descriptorWrites[0].dstBinding = 0;
    ...
    descriptorWrites[1].sType = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
    descriptorWrites[1].dstSet = descriptorSetsABS;
    ...
    descriptorWrites[2].sType = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
    descriptorWrites[2].dstSet = descriptorSetsABS;
...
std::vector<Sample> VisibilityManager::adaptiveBorderSample(const std::vector<Sample> &triangles) {
    ...
    // Copy intersected triangles from VRAM to CPU accessible memory
    std::vector<Sample> intersectedTriangles(triangles.size() * 9);
    {
        VkDeviceSize bufferSize = sizeof(intersectedTriangles[0]) * intersectedTriangles.size();
...
void VisibilityManager::rayTrace(const std::vector<uint32_t> &indices) {
    ...
    while (newSamples.size() >= MIN_ABS_RAYS) {
        ...
        int numbAbsRays = std::min(MAX_ABS_RAYS, newSamples.size());
