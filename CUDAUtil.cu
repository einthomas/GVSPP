...
void CUDAUtil::test() {
    /*
    thrust::device_vector<int> a(500000);
    thrust::device_vector<int> b(500000);
    thrust::device_vector<int> b(500000);
    int size = 500000;
...
auto s = std::chrono::steady_clock::now();
    auto end = thrust::lower_bound(a.begin(), a.end(), 0);

    int x = end - a.begin();
    if (x > 0) {
    //    thrust::remove(a, a + size, -1);
    }
auto e = std::chrono::steady_clock::now();
std::cout << "Total time: " << std::chrono::duration_cast<std::chrono::microseconds>(e - s).count() << "" << std::endl;
    */
}
